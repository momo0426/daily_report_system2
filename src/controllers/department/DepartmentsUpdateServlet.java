...
public class DepartmentsUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(_token != null && _token.equals(request.getSession().getId())) {
            ...
            Department dp = em.find(Department.class, (Integer)(request.getSession().getAttribute("department_id")));

            Boolean nameDuplicateCheckFlag = true;
            if(dp.getName().equals(request.getParameter("name"))) {
            ...
            dp.setName(request.getParameter("name"));
            dp.setUpdated_at(new Timestamp(System.currentTimeMillis()));

            List<String> errors = DepartmentValidator.validate(dp, nameDuplicateCheckFlag);
            if(errors.size() > 0) {
                em.close();
